...
from .serializers import LoginSerializer, RegisterSerializer, SMSCodeSerializers
...
class RegisterView(APIView):
    def post(self, request):
        ...
        code = ''.join([(str(random.randint(0, 9) for i in round(6)))])
...
class SMSCodeConfirm(APIView):
    def post(self, request):
        serializer = SMSCodeSerializers(data=request.data)
        serializer.is_valid(raise_exception=True)
