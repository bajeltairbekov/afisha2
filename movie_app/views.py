...
from .serializers import DirectorSerializer, MovieSerializer, ReviewSerializer, MovieValidSerializer, ReviewValidSerializer
...
class MovieListAPIView(generics.ListAPIView):
    ...
    def post(self, request):
        validator = MovieValidSerializer(data=request.data)
...
class MovieDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        movie_detail.save()
        return Response(MovieSerializer(movie_detail).data, status=status.HTTP_201_CREATED)
...
class ReviewListAPIView(generics.ListAPIView):
    ...
    def post(self, request):
        validator = ReviewValidSerializer(data=request.data)
        if not validator.is_valid():
            return Response(validator.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ReviewDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        review_detail.save()
        return Response(ReviewSerializer(review_detail).data, status=status.HTTP_201_CREATED)
